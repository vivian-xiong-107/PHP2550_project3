...
This report draws upon key insights from the paper *"Transporting a prediction model for use in a new target population"*. A pivotal aspect highlighted in the paper is the flexibility of model application irrespective of its precise specification. Specifically, the authors note that their results do not hinge on the prediction model being perfectly aligned with the true conditional expectation of the outcome in the source population, denoted as $E[Y|X,S=1]$ . In other words, the model $g_{\hat{\beta}}(X)$ does not necessarily have to converge to this true conditional expectation for it to be effective in application. [@steingrimsson2022]
...
```{r, echo=FALSE, message=FALSE, out.width="30%",warning=FALSE}
framingham_df_male = framingham_df %>% filter(SEX == 1)

par(mfrow= c(2,3))

ggplot(framingham_df_male, aes(x = AGE)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "lightblue") +
  geom_density() + 
  labs(title="Age in framingham dataset histogram plot")

ggplot(framingham_df_male, aes(x = SYSBP)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "lightpink") +
  geom_density() + 
  labs(title="SYSBP in framingham dataset histogram plot")


ggplot(framingham_df_male, aes(x = DIABP)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "orange") +
  geom_density()  + 
  labs(title="DIABP in framingham dataset histogram plot")


ggplot(framingham_df_male, aes(x = HDLC)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "mediumpurple") +
  geom_density() + 
  labs(title="HDLC in framingham dataset histogram plot")


ggplot(framingham_df_male, aes(x = BMI)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "palegreen1") +
  geom_density() + 
  labs(title="BMI in framingham dataset histogram plot")


ggplot(framingham_df_male, aes(x = TOTCHOL)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "orchid1") +
  geom_density() + 
  labs(title="TOTCHOL in framingham dataset histogram plot")
```
...
ggplot(framingham_df_female, aes(x = AGE)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "lightblue") +
  geom_density() + 
  labs(title="Age in framingham dataset histogram plot")

ggplot(framingham_df_female, aes(x = SYSBP)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "lightpink") +
  geom_density() + 
  labs(title="SYSBP in framingham dataset histogram plot")
...
ggplot(framingham_df_female, aes(x = DIABP)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "orange") +
  geom_density()  + 
  labs(title="DIABP in framingham dataset histogram plot")
...
ggplot(framingham_df_female, aes(x = HDLC)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "mediumpurple") +
  geom_density() + 
  labs(title="HDLC in framingham dataset histogram plot")
...
ggplot(framingham_df_female, aes(x = BMI)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "palegreen1") +
  geom_density() + 
  labs(title="BMI in framingham dataset histogram plot")
...
ggplot(framingham_df_female, aes(x = TOTCHOL)) + 
  geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "orchid1") +
  geom_density() + 
  labs(title="TOTCHOL in framingham dataset histogram plot")
...
names(missing_percentage)[names(missing_percentage) == "SYSBP_UT"] <- "SYSBP UT"
names(missing_percentage)[names(missing_percentage) == "SYSBP_T"] <- "SYSBP T"
```
...
The Brier risk, commonly used to assess the accuracy of probabilistic predictions, is a measure of how well a predictive model performs. It is particularly useful in evaluating models that output probabilities for binary outcomes. The Brier score ranges from 0 to 1, with lower values indicating better prediction accuracy. A Brier score of 0 means perfect prediction, while a score of 1 indicates the worst possible prediction.[@steyerberg2010] The Brier score (or Brier risk) between the NHANES and Framingham datasets is 0.6191495. A Brier score of approximately 0.62 suggests a moderate to high level of error in the probabilistic predictions made by the model. This indicates that the predicted probabilities diverge significantly from the actual outcomes. 

## Simulated transportability analysis

In order to simulate the \texttt{NHANES} dataset, we decided to generate all continuous variable (i.e., TOTCHOL, AGE, SYSBP, DIABP, HDLC, and BMI) follow to multivariate normal distribution. The covariance matrix ($\Sigma$) of these continuous variables from the \texttt{NHANES} dataset is meticulously computed. This matrix is crucial as it encapsulates the variance of each individual variable and the covariance amongst them, effectively preserving the inter-variable relationships observed in the original dataset. For this purpose, we consider each continuous variable ($X_{\text{continous}, i}$) as a random variable distributed according to a multivariate normal distribution, formally denoted as $\boldsymbol{X} \sim \mathcal{N}(\boldsymbol{\mu}, \Sigma)$, where $\boldsymbol{\mu}$ represents the mean vector and $\Sigma$ the covariance matrix. In our approach to Bernoulli variables, we generated these variables independently, rather than conditioning them on other variables. Specifically, \texttt{CURSMOKE}, \texttt{DIABETES}, \texttt{BPMEDS}, and \texttt{SEX} were each generated following a Bernoulli distribution. The probabilities for these distributions were determined based on the summary data presented in Table 3. This approach ensures that each variable is treated as a distinct entity, not influenced by the values of the others in the dataset. This disadvantage could be improved in further work.
